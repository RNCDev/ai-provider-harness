...
describe("togetherProvider", () => {
  it("streams via /chat/completions", async () => {
    ...
    const out = [];
    for await (const c of togetherProvider.chat({ modelId: "meta-llama/Llama-3-8b-chat-hf", messages: [{ role: "user", content: "hi" }] }, "together-key")) out.push(c);
    expect(out[0]?.type).toBe("text");
    expect(String(fetchSpy.mock.calls[0]![0])).toContain("api.together.xyz/v1/chat/completions");
  });
});
